...
def _execute_query(sql: str, max_rows: int = 50) -> str:
    ...
    if not stripped.startswith("SELECT") and not stripped.startswith("SHOW") and not stripped.startswith("DESCRIBE") and not stripped.startswith("DESC") and not stripped.startswith("EXPLAIN"):
